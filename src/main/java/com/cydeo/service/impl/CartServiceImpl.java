...
public class CartServiceImpl implements CartService {
    ...
    public void initialiseCartList() {
        ...
        cart1TotalAmount = cartItemList.stream()
                ...
                .reduce(cart1TotalAmount,BigDecimal::add);

//        for (CartItem cartItem : cartItemList){
//            cart1TotalAmount = cart1TotalAmount.add(cartItem.getProduct().getPrice()
...
//        }
        ...
        cart2TotalAmount = cartItemList1.stream()
                .map(item->item.getProduct().getPrice().multiply(BigDecimal.valueOf(item.getQuantity())))
                .reduce(cart2TotalAmount,BigDecimal::add);

//        for (CartItem cartItem : cartItemList1){
//            cart2TotalAmount = cart1TotalAmount.add(cartItem.getProduct().getPrice()
//                    .multiply(BigDecimal.valueOf(cartItem.getQuantity())));
//        }
